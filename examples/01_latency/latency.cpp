    ...
    std::size_t window_size)
{
    ...
    for (std::size_t i = 0; i < loop + skip; ++i) {
        ...
        auto range = boost::irange(start, window_size);
        for_each(par.with(cs), boost::begin(range), boost::end(range),
            [&](std::uint64_t j)
            {
                msg(dest,
        ...
        hpx::async(msg, dest,
            buffer_type(send_buffer, size, buffer_type::reference)).then(
                ...
                [&,parcel_count](auto &&f) -> void {
                    ...
                    // std::cout << "Signalled with value " << parcel_count << "\n";
                }
            );

        //
        // std::cout << "Waiting with value " << parcel_count << "\n";
...
//        " wait is " << parcel_count-2+window_size << "\n";
    sem.wait(parcel_count - 2 + window_size);
...
void run_benchmark(boost::program_options::variables_map & vm)
{
    ...
    {
        double latency = receive_new(there, send_buffer, size, loop, window_size);
